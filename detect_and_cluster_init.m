function [clusterDetails,spike_features,sum_fig] = detect_and_cluster_init(sfile,params,use_chs, varargin)

if nargin < 3 || isempty(use_chs)
    use_chs = nan;
end

%% BGC  Allow passing of triggered Spikes, so just do clustering
Spikes = [];

j = 1; 
while j <= length(varargin)
    if isfield(varargin{j},'trig_vals')
        Spikes = varargin{j};
    end
    j = j+1;
end
...
elseif isfield(sfile,'V') %BGC. Can sometimes be missing. 
    V = sfile.V(:,use_chs);
    Vtime = sfile.Vtime;
    Fs = sfile.Fs;
else %BGC
    fprintf('No V data in %s\n',sfile);
    V = [];
    return;
end
%% DETECT SPIKES
...
%if spikes is not empty, triggering has been done elsewhere.
if isempty(Spikes)
...
if trig_thresh/noise_sigma >= params.noise_thresh && isempty(Spikes)
...
Spikes = getSpikeSnippets(V,Vtime,spk_id,params.spk_pts,trig_ch);
    ...
    spk_id = 1:length(Spikes.V);
    trig_thresh = Spikes.trig_thresh;
end
% artifact detection
artifact_ids = find_spike_artifacts(Spikes,params);
uids = setdiff(1:size(Spikes.V,1),artifact_ids); %bgc return used events
...
clusterDetails.uids = uids; %BGC return event/artifact ids.
clusterDetails.artifact_ids = artifact_ids;
